        ...
        public async Task<UpdateAWSCredentialResponse> UpdateAsync(string id, UpdateAWSCredentialRequestBody? requestBody = null, RetryConfig? retryConfig = null)
        {
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    try
                    {
                        obj = ResponseBodyDeserializer.DeserializeNotNull<AWSCredential>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into AWSCredential.", httpRequest, httpResponse, httpResponseBody, ex);
                    }
