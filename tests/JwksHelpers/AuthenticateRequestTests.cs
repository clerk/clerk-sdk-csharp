...
namespace JwksHelpers.Tests
{
    ...
    public class AuthenticateRequestTests : IClassFixture<JwksHelpersFixture>
    {
        ...
        public async Task TestApiGatewayScenario()
        {
            ...
            // Should accept both session and machine tokens
            var tokens = new[]
            {
                "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.session_token",
                "mt_service_token_123",
                "oat_oauth_token_123",
                "ak_api_key_123"
            };
        ...
        [Fact]
        public async Task TestMultiTokenTypeAuthentication()
        {
            // Test that the same endpoint can handle different token types with mixed acceptance
            var arOptions = new AuthenticateRequestOptions(
                secretKey: "sk_test_secret",
                acceptsToken: new[] { "session_token", "oauth_token" }
            );

            var testCases = new[]
            {
                ("mt_machine_token", false), // Should be rejected
                ("oat_oauth_token", true), // Should be accepted
                ("ak_api_key", false), // Should be rejected
                ("session_jwt", true) // Should be accepted
            };

            foreach (var (token, shouldBeAccepted) in testCases)
            {
                var httpContext = CreateHttpContextWithToken(token);
                var state = await AuthenticateRequest.AuthenticateRequestAsync(httpContext.Request, arOptions);
        ...
        [ConditionalFact("CLERK_SECRET_KEY", "CLERK_SESSION_TOKEN")]
        public async Task TestBackwardCompatibilityWithExistingSessionTokens()
        {
            // Ensure existing session token functionality still works with new acceptsToken parameter
            var arOptions = new AuthenticateRequestOptions(
                secretKey: _fixture.SecretKey,
                audiences: _fixture.Audiences,
                authorizedParties: _fixture.AuthorizedParties,
                acceptsToken: new[] { "session_token" } // Explicitly only session tokens
            );
        ...
        [ConditionalFact("CLERK_JWT_KEY", "CLERK_SESSION_TOKEN")]
        public async Task TestBackwardCompatibilityWithJwtKey()
        {
            // Ensure JWT key verification still works
            var arOptions = new AuthenticateRequestOptions(
                jwtKey: _fixture.JwtKey,
                audiences: _fixture.Audiences,
                authorizedParties: _fixture.AuthorizedParties,
        ...
        [Fact]
        public async Task TestDefaultAcceptsTokenBehavior()
        {
            // When acceptsToken is not specified, should default to "any" (backward compatible)
            var arOptions = new AuthenticateRequestOptions(secretKey: "sk_test_secret");
